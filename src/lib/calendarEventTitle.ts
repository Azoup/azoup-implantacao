 ...
 *
 * - **EMPRESA** — nome do projeto (`projectName` → `tradeName` se o nome estiver vazio).
...
/**
 * Nome do projeto para o prefixo EMPRESA (não usa razão social).
...
export function extractEmpresaFromProject(project: CalendarTitleProject | null | undefined): string {
  const raw = (project?.projectName?.trim() || project?.tradeName?.trim() || '').trim()
  if (!raw) return ''
